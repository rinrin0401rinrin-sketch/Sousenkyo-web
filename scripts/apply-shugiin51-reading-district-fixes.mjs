...
const fixesPath = path.join(reviewRoot, "manual-fix", "medium-reading-district-fix-candidates.csv");
const reportPath = path.join(reviewRoot, "manual-fix", dryRun ? "medium-reading-district-apply-dry-run.csv" : "medium-reading-district-apply-report.csv");
...
for (const row of candidates) {
  ...
  if (!fix) continue;
  const beforeReading = row.reading;
  const beforeDistrict = row.districtLabel;
...
console.log(`${dryRun ? "Dry-run checked" : "Applied"} ${reportRows.length} reading/district fix(es)`);
